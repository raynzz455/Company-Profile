<script lang="ts">
  ...
  function handleFileChange(event: Event) {
    ...
    if (file) {
      const confirmUpload = confirm(
        `Apakah kamu yakin untuk mengupload "${file.name}" ini?`
      );
      if (confirmUpload) {
        isLoading = true;
        input.form?.submit();
  ...
  async function handleDelete(imageUrl: string | undefined) {
    ...
    const confirmDelete = confirm(
      `Apakah kamu yakin ingin menghapus "${imageName}"?`
    );
  ...
  async function handleDownload(imageUrl: string | undefined) {
    if (!imageUrl) {
        alert("URL gambar tidak valid.");
        return;
    }
...
<div class="bg-white max-h-screen sm:max-w-[1000px] mx-auto">
  <p
    class="text-left ml-5 sm:ml-6 text-sm md:text-lg font-medium uppercase my-4"
  >
    Portofolio
  </p>
  ...
  <div class="max-h-[90vh] w-full">
    <div
      class="bg-gray-100 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sm:rounded-tl-2xl sm:rounded-tr-2xl flex flex-row justify-between"
    >
      <div class="flex flex-row m-3">
        <p class="px-4 sm:px-5 flex text-left py-3">Images</p>
        <select
          class="bg-transparent justify-center"
        ...
        >
          ...
          >
          <option value="4" selected={data.folder === "4"}
      ...
      <div class="flex flex-row gap-2">
        ...
        <form
          ...
          class="flex relative sm:hidden items-center justify-center w-8 h-8 mt-4 mr-2 rounded-full bg-[#f28928] hover:bg-[#c97b31] text-white"
        >
        ...
        <form
          method="post"
          enctype="multipart/form-data"
          class="input-images-pc relative hidden sm:flex items-center justify-center h-10 mt-3 mr-3 pr-2 pl-4 bg-[#f28928] hover:bg-[#c97b31] text-white rounded-3xl"
        >
          <input type="hidden" name="folder" value={selectedFolder} />
          <input
            type="file"
    ...
    <div
      class="w-full h-auto p-2 flex flex-col gap-4 sm:py-4 sm:grid sm:grid-cols-3"
    >
        ...
        <div class="border w-full sm:w-[300px] h-[230px] sm:h-[200px] mx-auto">
          <div class="w-full h-full overflow-hidden group relative">
            <img
              src={image}
              alt="gambar"
              class="gambarlah w-full h-full object-cover object-center transition-transform duration-[350ms] group-hover:scale-110"
            />
            {#if isCheckboxChecked}
              <div in:fade={{ duration: 200 }} out:fade={{ duration: 200 }}>
                <Fun on:delete={() => handleDelete(image)} on:download={() => handleDownload(image)}/>
              </div>
            {/if}
          </div>
        </div>
